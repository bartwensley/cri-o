...
@test "test cpu load balancing per-container annotation targets specific container" {
	...
	cat "$TESTDIR"/sbox.json
	cat "$TESTDIR"/ctr_a.json
	cat "$TESTDIR"/ctr_b.json

	pod_id=$(crictl runp --runtime high-performance "$TESTDIR"/sbox.json)
	ctr_a=$(crictl -D create "$pod_id" "$TESTDIR"/ctr_a.json "$TESTDIR"/sbox.json)
	crictl start "$ctr_a"
...
@test "test cpu load balancing pod-level annotation applies to all containers" {
	...
	ctr_b=$(crictl -D create "$pod_id" "$TESTDIR"/ctr_b.json "$TESTDIR"/sbox.json)
	crictl start "$ctr_b"
